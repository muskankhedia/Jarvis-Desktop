...
try {
	...
	speakWorker = new Worker('../dependencies/speakWorker.js');
...
function speak(text, args) {
	...
	function playAudioDataAPI(data) {
		...
		try {
			...
			let output = new Audio();
			output.mozSetup(1, data.sampleRate);
			let num = data.samples.length;
			let buffer = data.samples;
			let f32Buffer = new Float32Array(num);
			for (let i = 0; i < num; i++) {

				let value = buffer[ i << 1 ] + (buffer[ (i << 1) + 1 ] << 8);
				if (value >= 0x8000) {
	...
	function handleWav(wav) {
		...
		let data = parseWav(wav); // validate the data and parse it
	...
	if (args && args.noWorker) {
		...
		let wav = generateSpeech(text, args);
		if (PROFILE) {
